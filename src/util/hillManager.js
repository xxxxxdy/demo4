...
export class NewCode{
    ...
    getJson(idx, persistent = 0){
        ...
        if(mapping_relationship["using-line"]){
            ...
            if(mapping_relationship["line1"]["y"] !== "none" && mapping_relationship["line1"]["strokeWidth"] !== "none"){
                let y_key = mapping_relationship["line1"]["y"];
                for(let i=0; i<data_1code["values"].length; i++){
                    data_1code["values"][i]["l1y_draw"] =
                        (1-data_1code["values"][i][y_key])*(CODE_HEIGHT-data_1code["values"][i]["l1sw_draw"]) +
                        data_1code["values"][i]["l1sw_draw"]/2 + y_offset;
                }
                marks_line1["encode"]["enter"]["y"]={"field": "l1y_draw"};
                marks_line1["encode"]["enter"]["y2"]={"field": "l1y_draw"};
            }
        ...
        if(mapping_relationship["using-rect"]){
            ...
            for(key in mapping_relationship["rect"]){
                if(mapping_relationship["rect"][key] === "none"){
                    switch(key){
                        case "x": // 为了防止越界，先不管，后面再单独处理
                            break;
                        case "width": // 可能越界，后续调整
            ...
            // 把这玩意嵌套在最外层是因为这玩意最有可能被优化出去
            if(mapping_relationship["rect"]["width"] === "none"){
                let rect_width = CODE_WIDTH/data_1code["values"].length*mapping_default["rect"]["width"];
                if(mapping_relationship["using-line"]){
                    if(mapping_relationship["rect"]["x"] === "none"){
                        for(let i=0; i<data_1code["values"].length; i++){
                            data_1code["values"][i]["rx_draw"] = data_1code["values"][i]["l1x_draw"] + 
                              (data_1code["values"][i]["l1x2_draw"]-data_1code["values"][i]["l1x_draw"]-rect_width)/2;
                        }
                    }
                    else{
                        ...
                        for(let i=0; i<data_1code["values"].length; i++){
                            data_1code["values"][i]["rx_draw"] = data_1code["values"][i]["l1x_draw"] + 
                            (data_1code["values"][i]["l1x2_draw"]-data_1code["values"][i]["l1x_draw"]-rect_width) * data_1code["values"][i][x_key];
                        }
                    }
                }
                else{
                    if(mapping_relationship["rect"]["x"] === "none"){
                        for(let i=0; i<data_1code["values"].length; i++){
                            data_1code["values"][i]["rx_draw"] = data_1code["values"][i]["left"]*CODE_WIDTH +
                              (data_1code["values"][i]["width"]*CODE_WIDTH -rect_width)/2 + CODE_PADDING;
                        }
                    }
                    else{
                        let x_key = mapping_relationship["rect"]["x"];
                        for(let i=0; i<data_1code["values"].length; i++){
                            data_1code["values"][i]["rx_draw"] = data_1code["values"][i]["left"]*CODE_WIDTH +
                             (data_1code["values"][i]["width"]*CODE_WIDTH -rect_width) * data_1code["values"][i][x_key] + CODE_PADDING;
                        }
                    }
                }
            }
                ...
                if(mapping_relationship["using-line"]){
                    if(mapping_relationship["rect"]["x"] === "none"){
                        for(let i=0; i<data_1code["values"].length; i++){
                            data_1code["values"][i]["rx_draw"] = data_1code["values"][i]["l1x_draw"] + 
                              (data_1code["values"][i]["l1x2_draw"]-data_1code["values"][i]["l1x_draw"]-
                              data_1code["values"][i]["rwidth_draw"])/2;
                        }
                    }
                    else{
                        let x_key = mapping_relationship["rect"]["x"];
                        for(let i=0; i<data_1code["values"].length; i++){
                            data_1code["values"][i]["rx_draw"] = data_1code["values"][i]["l1x_draw"] + 
                            (data_1code["values"][i]["l1x2_draw"]-data_1code["values"][i]["l1x_draw"]-
                            data_1code["values"][i]["rwidth_draw"]) * data_1code["values"][i][x_key];
                        }
                    }
                }
                else{
                    if(mapping_relationship["rect"]["x"] === "none"){
                        for(let i=0; i<data_1code["values"].length; i++){
                            data_1code["values"][i]["rx_draw"] = data_1code["values"][i]["left"]*CODE_WIDTH +
                              (data_1code["values"][i]["width"]*CODE_WIDTH -
                              data_1code["values"][i]["rwidth_draw"])/2 + CODE_PADDING;
                        }
                    }
                    else{
                        let x_key = mapping_relationship["rect"]["x"];
                        for(let i=0; i<data_1code["values"].length; i++){
                            data_1code["values"][i]["rx_draw"] = data_1code["values"][i]["left"]*CODE_WIDTH +
                             (data_1code["values"][i]["width"]*CODE_WIDTH -
                             data_1code["values"][i]["rwidth_draw"]) * data_1code["values"][i][x_key] + CODE_PADDING;
                        }
                    }
                }
            }
